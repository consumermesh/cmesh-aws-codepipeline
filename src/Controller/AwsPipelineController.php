...
class AwsPipelineController extends ControllerBase {
  ...
  public function trigger(Request $request) {
    ...
    try {
      ...
      // nulls into the build, and the build has no way to tell that apart from
  ...
  /**
   * API endpoint reporting whether every GraphQL field still has a resolver.
   *
   * Read-only on purpose. Repairs run from cron and immediately before a
   * content push, so a monitor polling this endpoint cannot clear caches.
   *
   * @return \Symfony\Component\HttpFoundation\JsonResponse
   *   JSON response listing any fields that resolve to null with no error.
   */
  public function graphqlHealth() {
    $missing = cmesh_aws_pipeline_graphql_schema_check(FALSE);
